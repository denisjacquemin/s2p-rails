Rpush.configure do |config|
  ...
  config.log_level = :debug #(defined?(Rails) && Rails.logger) ? Rails.logger.level : ::Logger::Severity::INFO
...
Rpush.reflect do |on|
  ...
  on.ssl_certificate_will_expire do |app, expiration_time|
    Rails.logger.info "[Rpush.reflect ssl_certificate_will_expire] certificate (app #{app})will expire within 1 month (#{expiration_time})"
  end

  # Called when an SSL certificate has been revoked.
  on.ssl_certificate_revoked do |app, error|
    Rails.logger.info "[Rpush.reflect ssl_certificate_revoked] SSL certificate has been revoked (app: #{app} error: #{error})"
  end
  ...
  on.error do |error|
    Rails.logger.info "[Rpush.reflect error] #{error}"
  end
end
