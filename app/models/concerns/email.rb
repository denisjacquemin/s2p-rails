module Email extend ActiveSupport::Concern
    ...
    def sendMessageByEmail(message, delivery_method)
        ...
        if has_recipients?(message)
            student_ids = find_student_ids(message.groups, [message.students, message.recipients.pluck(:student_id)].compact.reduce([], :|), message.school.iscity?, message.message_categories.pluck(:id))
