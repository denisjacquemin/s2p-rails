class MessagesController < ApplicationController
  ...
  def index
    ...
    @latest_messages = policy_scope(Message).includes(:author).where(school_id: current_school.id).order(created_at: :desc).limit(12).pluck(:id, :title, :has_form, :content, "CONCAT_WS(' ', users.firstname, users.lastname)", :updated_at, :status)
  end
